...
$x=sequence(2);
$y=$x->cplx;
ok(ref $y eq 'PDLA::Complex', 'type of cplx');
ok(ref $x eq 'PDLA', "cplx doesn't modify original pdl");
$z=$y->real;
ok(ref $z eq 'PDLA', 'real returns type to pdl');
ok(ref $y eq 'PDLA::Complex', "real doesn't change type of parent");
#Should there be a real subroutine, such as complex, that does change
#the parent?
$y=$x->complex;
ok(ref $y eq 'PDLA::Complex', 'type of complex');
ok(ref $x eq 'PDLA::Complex', 'complex does modify original pdl');

#Check r2C
ok(ref r2C(1) eq 'PDLA::Complex', 'type of r2C');
is(r2C(1)->re, 1, 'real part of r2C');
is(r2C(1)->im, 0, 'imaginary part of r2C');

#Check i2C
ok(ref i2C(1) eq 'PDLA::Complex', 'type of i2C');
...
ok(ref $x eq PDLA::Complex, 'type promotion i - real scalar');
ok(tapprox($x->real,$ref), 'value from i - real scalar');

$x = 1-i;
ok(ref $x eq PDLA::Complex, 'type promption real scalar - i');
ok(tapprox($x->real,-$ref), 'value from real scalar - i');

$ref = pdl([[-2,1],[-3,1]]);
$x = i - pdl(2,3);

ok(ref $x eq PDLA::Complex, 'type promotion i - real piddle');
ok(tapprox($x->real,$ref), 'value from i - real piddle');

$x = pdl(2,3) - i;
ok(ref $x eq PDLA::Complex, 'type promption real piddle - i');
...
ok(ref Cabs $x eq 'PDLA', 'Cabs type');
ok(ref Cabs2 $x eq 'PDLA', 'Cabs2 type');
ok(ref Carg $x eq 'PDLA', 'Carg type');
...
#rCpolynomial
 TODO: {
     local $TODO="rCpolynomial doesn't return a PDLA::Complex";
     ok(ref rCpolynomial(pdl(1,2,3), $x) eq 'PDLA::Complex',
	'rCpolynomial type');
}
...
$y = $x->copy + 1;
my $bigArray = $x->cat($y);
ok(abs($bigArray->sumover->sumover +  8 - 4*i) < .0001, 'check cat for PDLA::Complex');
 SKIP: {
...
$y=$x->copy->complex;
 SKIP: {
     ...
     ok(ref $y->Csumover eq 'PDLA::Complex', 'Type of Csumover');
     is($y->Csumover->dim(0), 2, 'Dimension 0 of Csumover');
     ok(tapprox($y->Csumover->real, $x->mv(1,0)->sumover),
	'Csumover value');
}
ok(ref $y->sumover eq 'PDLA::Complex', 'Type of sumover');
...
ok(tapprox($y->sumover->real, $x->mv(1,0)->sumover), 'sumover value');
 TODO: {
     ...
     ok(ref sumover($y) eq 'PDLA::Complex', 'Type of sumover');
...
#Check sum
 TODO: {
     local $TODO="Complex sum works miraculously only for 2,n,m PDLA's";
   SKIP: {
       todo_skip "Complex sum for 2,n Complex PDLA's doesn't work", 4;
       $x=sequence(2,3)+1;
       $y=$x->copy->complex;
       ok(ref $y->sum eq 'PDLA::Complex', 'Type of sum');
...
#Check prod
 TODO: {
   ...
   SKIP: {
       ...
       $x=sequence(2,3)+1;
       $y=$x->copy->complex;
       ok(ref $y->prod eq 'PDLA::Complex', 'Type of prod');
...
ok(tapprox(Cabs(atan2(pdl(1)->r2C,pdl(0)->r2C)),PDLA::Math::asin(1)),"atan2");
...
TODO: {
      ...
      $y=$x->mv(0,1);
      ok(ref $y eq 'PDLA', 'PDLA::Complex becomes real PDLA after moving dim 0');
}
