...
$a = zeroes(0);
ok($a->nelem==0,"you can make an empty PDLA with zeroes(0)");
ok("$a" =~ m/Empty/, "the empty PDLA looks empty");
...
$b = $a->slice(":,:");
eval { $a->reshape(4); };
ok( $@ =~ m/Can\'t/, "reshape fails on a PDLA with a child" );

eval { $b->reshape(4); };
ok($@ =~ m/Can\'t/, "reshape fails on a PDLA with a parent" );
