...
so that
	...
	$x = some piddle
	$b = $x->slice("some parts");
	$b->set(3,3,10);

also changes the corresponding element in $x. $b has become effectively
...
Consider the following code ($x and $b are pdls that have dataflow enabled):
