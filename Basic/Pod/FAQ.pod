...
This is version 1.005 of the PDLA FAQ, a collection of  frequently 
...
writing PDLA requires version 5.8 of perl, or higher.  More
...
PDLA in the directory /modules/by-module/PDLA/. PDLA's homepage is at
...
C<pdl2> session of a PDLA beginner (lengthy output is only symbolically
reproduced in braces ( <... ...> ) ):
    ...
    < ... perldoc page ... >
    pdla> $a = pdl (1,5,7.3,1.0)
    pdla> $b = sequence float, 4, 4
    pdla> help inner
...
If you are interested in PDLA in general you can join the PDLA mailing
list pdl-general@lists.sourceforge.net. This is a forum to discuss programming
issues in PDLA, report bugs, seek assistance with PDLA related problems,
...
development you can join the PDLA developers mailing list
...
purposes and more general availability.  The current post-PDLA-2.007 developers
release of PDLA is available as F<CHM/PDLA-2.007_01.tar.gz> .
...
minimum PDLA version of 2.2.  The work around, it to note that all extant
PDLA releases have version numbers greater than 2.2.1 so that just using
...
programs C<pdl, pdladoc, pdl2, perldla, and pptemplate>.  Then just build
...
might not always be the latest version.  PDLA-2.007 builds out of
...
accessed via the Tickets menu at PDLA's Sourceforge project page
...
and they won't be happy to have their recent PDLA fail to build!
...
=head2 Q: 6.1    What is perldla?   What is pdl2?


Sometimes C<perldla> (C<pdl2>) is used as a synonym for PDLA. Strictly
speaking, however, the name C<perldla> (C<pdl2>) is reserved for the
little shell that comes with the PDLA distribution and is
supposed to be used for the interactive prototyping of PDLA
scripts. For details check L<perldla> or L<pdl2>.
...
Also try the C<demo> command in the C<perldla> or C<pdl2>
...
using lvalue subroutines:
    ...
    pdla> $a = sequence(5); p $a
    [0 1 2 3 4]
    pdla> $a->slice('1:2') .= pdl([5,6])
    pdla> p $a
    [0 5 6 3 4]
...
all elements in a piddle fulfills some criterion:
    ...
    pdla> $a=pdl ( 1, -2, 3);
    pdla> print '$a has at least one element < 0' if (any $a < 0);
    $a has at least one element < 0
    
    pdla> print '$a is not positive definite' unless (all $a > 0);
...
C<help PDLA::Objects> in the I<perldla> or I<pdl2> shell).
...
On CPAN you can find the L<PDLA::NetCDF|PDLA::NetCDF> module that works with the current
released version of PDLA 2.007.
...
C<perldla> or C<pdl2> interactive shell:

    perl -Mblib perldla  or  perl -Mblib Perldl2/pdl2
...
The first stop is again C<perldla> or C<pdl2> and the on-line help
