...
initiator of the PDLA project:
    ...
    of scientific computing. e.g. $x=$b+$c can add two
...
C<inner $x, $b> of a (3) pdl C<$x> and a (3,5,4) pdl 
...
C<$x =  pdl([$b, 0, 2])> , but this does not work. The way to
...
then both $b and $x point to the same object, e.g. then saying
