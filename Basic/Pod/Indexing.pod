...
[ work in the new index function: C<$b = $x-E<gt>index($c);> ???? ]
...
its number in the parent) to a new position in the new child piddle:
 ...
 $b = $x->mv(4,0);  # make the 5th dimension of $x the first in the
                    # new child $b
...
C<func((n,m),[o](n))> with a piddle C<a(2,4,7,1,3)> as C<f($x,($o = null))>
...
is derived from C<($x,$b,$c)> in a way that can be expressed in pdl
pseudo-code as
...
"functional" style but rather say
 ...
 $result = null;  
 for (0...$n) {
      func($x,$b,$result); # in all but the first invocation $result
...
created) as
 ...
 func($x->thread(1,3),$b->thread(0,3),$c,$d->thread(0,1))
...
Example 2:
 ...
 $res=zeroes(($x->dims)[0],($b->dims)[0]);
 my_biop1($x->thread(0,-1),$b->thread(-1,0),$res->(0,1),"*");
 # similar thing by implicit threading with auto-created piddle
 $res = $x->dummy(1) * $b->dummy(0);
