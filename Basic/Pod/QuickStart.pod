...
Perl code C<$x = $b + $c>, where C<$b> and C<$c> are large datasets
...
C<$x = $b + 2> does a vector operation on each value in the
spectrum/image/etc.
...
=head2 Arithmetic (and boolean expressions)
 ...
 $c=sqrt($x); $d = log10($b+100); # Etc

 $e = $x>42; # Vector conditional

 $e = 42*($x>42) + $x*($x<=42); # Cap top

 $b = $x->log10 unless any ($x <= 0); # avoid floating point error
...
via a null-method modifier to a PDLA:
  ...
  $b = $x->($x1:$x2,$y1:$y2,($z1)); # Take subsection

Here, C<$x> is a 3-dimensional variable, and C<$b> gets a planar
...
Some other miscellany:
 ...
 $b = xvals($x); # Fill array with X-coord values (also yvals(), zvals(),
...
Also if one says:
 ...
 $b = $x;     # $b and $x now point to same data
 $x = $x + 1;

Then C<$b> and C<$x> end up being different, as one naively expects,
...
needed - the downside of this is that if C<$b=$x> then C<$b> is also
