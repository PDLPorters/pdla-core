...
use PDL::Core;
use PDL::Exporter;
use PDL::Primitive::Inline Pdlpp => Config => clean_after_build => 0;
use PDL::Primitive::Inline Pdlpp => 'DATA';
use parent 'PDL::Exporter';
use PDL::Slices;
use Carp;

our @EXPORT_OK = qw(uniqvec fibonacci random randsym grandom vsearch which which_both interpol);
our %EXPORT_TAGS = (Func=>\@EXPORT_OK);

for my $func (qw(
    inner outer matmult innerwt inner2 inner2d inner2t crossp norm
    indadd conv1d in uniq uniqind hclip lclip clip wtstat
    statsover stats histogram whistogram histogram2d whistogram2d
    append axisvalues vsearch_sample
    vsearch_insert_leftmost vsearch_insert_rightmost vsearch_match
    vsearch_bin_inclusive vsearch_bin_exclusive interpolate
    interpND one2nd where whereND whichND setops intersect
)) {
    import_export($func);
}
...
sub PDL::whichND {
  ...
  my $ind = $mask->flat->which->dummy(0,$mask->getndims)->long->make_physical;
  if($ind->nelem==0) {
