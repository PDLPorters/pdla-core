...
The C<mode> option indicates which method of searching to use (default:
C<sample>), and may be one of:
...
Search for values in a sorted array, return index appropriate for sampling
from a distribution
...
inclusive to it.  B<vsearch_bin_exclusive> returns an index I<I> for
each value I<V> of C<$vals>.
...
sub PDLA::setops {
  ...
  if ($op eq 'OR') {
    ...
    my $i1 = which($union != PDLA::Slices::rotate($union, 1));
    my $i2 = which($union != PDLA::Slices::rotate($union, -1));
    #
    ...
    return $union->where($union == PDLA::Slices::rotate($union, -1));
  } else {
...
pp_def('matmult',
	...
	PMCode => '#', # triggers the _int thing as true
	Code => <<'EOC',
