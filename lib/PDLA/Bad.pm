...
PDLA::Bad - PDLA can process bad values if configured at compile-time
...
If PDLA is compiled with C<WITH_BADVAL> set to 1,
you can enter the wonderful world of bad value support in
PDLA.

If not, it will not contain any bad-value support code. Actually, a
number of methods are defined, but they are only placeholders to make
writing other code, that has to handle WITH_BADVAL being true or false,
easier.

This module is loaded when you do C<use PDLA>,
C<use PDLA::Lite> or C<PDLA::LiteF>.

Implementation details are given in L<PDLA::BadValues>.
...
Set to C<$PDLA::Config{WITH_BADVAL}>.
...
If C<$PDLA::Bad::UseNaN == 1>, this is just a copy
